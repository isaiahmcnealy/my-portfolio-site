  ...
  <section id="experience" className="flex flex-col items-center py-20 bg-gray-900 text-white">
    ...
    <div className="max-w-3xl w-full">
      {experiences.map((exp, index) => (
        <div key={index} className="hover:bg-opacity-50 mb-8 border-l-4 border-teal-500 pl-4">
          ...
          {exp.skills && (
            <div className="flex flex-wrap mt-2">
              {exp.skills.map((skill, i) => (
                <span key={i}
                      className="bg-teal-500 text-gray-900 py-1 px-3 rounded-full text-sm font-semibold mb-2 mr-2">
                  {skill}
                </span>
              ))}
            </div>
